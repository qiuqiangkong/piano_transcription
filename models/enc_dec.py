...
class EncDec(nn.Module):
    def __init__(self, config: EncDecConfig) -> None:
        ...
        self.audio_emb_to_emb = nn.Linear(config.audio_n_embd, config.n_embd, bias=True)
    ...
    def forward(
        self, audio_emb, idx: torch.Tensor, target=None, target_mask=None, max_seq_length: Optional[int] = None, input_pos: Optional[torch.Tensor] = None
        ...
        if input_pos is None:  # proxy for use_cache=False
            for block in self.transformer.h:
                x, _ = block(x, audio_emb, rope, mask, max_seq_length)
        else:
    ...
    @classmethod
    def from_name(cls, name: str) -> Self:
        return cls(EncDecConfig.from_name(name))
...
class EncDecPos(nn.Module):
    def __init__(self, config: EncDecConfig) -> None:
        ...
        self.poss = AbsolutePositionalEncoder(emb_dim=64, max_position=2048)
        self.pos_fc = nn.Linear(64, config.n_embd)
    ...
    def forward(
        ...
        audio_h = self.audio_emb_to_emb(audio_emb)
        audio_pos_h = self.pos_fc(self.poss(audio_h).to(audio_h.device))

        sum_h = audio_h + audio_pos_h
        ...
        if input_pos is None:  # proxy for use_cache=False
            for block in self.transformer.h:
                x, _ = block(x, sum_h, rope, mask, max_seq_length)
        else:
            if not self.kv_caches:
                head_size = self.config.n_embd // self.config.n_head
                cache_shape = (B, self.config.n_head, max_seq_length, head_size)
        ...
        if target is not None:
            target *= target_mask
            logits = logits[:, -target.shape[1] :, :].contiguous()
            # tar = target.contiguous().view(-1).clone()
            # tar = target.view(-1)
            # loss = F.cross_entropy(logits.view(-1, logits.size(-1)), tar, ignore_index=0)
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), target.view(-1), ignore_index=0)
        else:
            logits = self.lm_head(x[:, [-1], :]) # note: using list [-1] to preserve the time dim
            loss = None
...
class Block(nn.Module):
    def __init__(self, config: EncDecConfig) -> None:
        super().__init__()
        self.self_rms_1 = RMSNorm(config.n_embd)
        self.self_attn = CausalSelfAttention(config)
        self.self_rms_2 = RMSNorm(config.n_embd)
        self.self_mlp = MLP(config)

        self.cross_rms_1 = RMSNorm(config.n_embd)
        self.cross_attn = CrossAttention(config)
        self.cross_rms_2 = RMSNorm(config.n_embd)
        self.cross_mlp = MLP(config)
    ...
    def forward(
        ...
        h, new_kv_cache = self.self_attn(self.self_rms_1(x), rope, mask, max_seq_length, input_pos, kv_cache)
        x = x + h
        x = x + self.self_mlp(self.self_rms_2(x))

        h, new_kv_cache = self.cross_attn(self.cross_rms_1(x), enc_h, rope, max_seq_length, input_pos, kv_cache)
        x = x + h
        x = x + self.cross_mlp(self.cross_rms_2(x))
...
class CrossAttention(nn.Module):
    def __init__(self, config: EncDecConfig) -> None:
        ...
        # key, query, value projections for all heads, but in a batch
        self.c_attn = nn.Linear(config.n_embd, 1 * config.n_embd, bias=False)
    ...
    def forward(
        ...
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        q = self.c_attn(x)
        k, v = self.c_attn2(enc_h).split(self.n_embd, dim=2)
        
        head_size = C // self.n_head
        k = k.view(B, enc_T, self.n_head, head_size)
        q = q.view(B, T, self.n_head, head_size)
        v = v.view(B, enc_T, self.n_head, head_size)
