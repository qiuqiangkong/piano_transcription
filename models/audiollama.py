...
class AudioLlama(nn.Module):
    ...
    def forward(
        self, audio_emb, idx: torch.Tensor, target=None, max_seq_length: Optional[int] = None, input_pos: Optional[torch.Tensor] = None
        ...
        if target is not None:
            logits = logits[:, audio_emb.shape[1] :, :].contiguous()
    ...
    @torch.no_grad()
    def generate(self, audio_emb, idx, max_new_tokens, temperature=1.0, top_k=None):
        """
