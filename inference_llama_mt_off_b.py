...
from train_off import get_model
import mir_eval
import re
...
def inference_in_batch(args):
    ...
    # Load checkpoint
    enc_model_name = "CRnn3_off"
    checkpoint_path = Path("checkpoints/train_llama_mt_off_b/AudioLlama/step=50000_encoder.pth")
    ...
    # Load checkpoint
    checkpoint_path = Path("checkpoints/train_llama_mt_off_b/AudioLlama/step=50000.pth")
    config = LLaMAConfig(
    ...
    # for audio_idx in range(110, len(audio_paths)):
        ...
        while bgn_sec < audio_seconds:
            ...
            for note in pred_onset_notes:
                ...
                # if note.end < bgn_sec or end_sec < note.start:
                #     pass
                # else:
                #     candidate_notes.append(note)
            ...
            ### Sustain notes
            while pointer < len(sustain_notes):

                _N = min(batch_size, len(sustain_notes) - pointer)
                ...
                with torch.no_grad():
                    enc_model.eval()
                    audio_emb = enc_model(segments)["offset_emb"]

                q_tokens = []
                a_tokens = []
                
                for note in sustain_notes[pointer : pointer + batch_size]:
                ...
                for k in range(len(pred_tokens)):
                    name = tokenizer.itos(pred_tokens[k][1])
                    
                    if name == "name=note_off":
                        offset_time = tokenizer.itos(pred_tokens[k][2])
                        offset_time = float(re.search('time=(.*)', offset_time).group(1))

                        note = pretty_midi.Note(
                            pitch=sustain_notes[pointer + k]["pitch"], 
                            start=sustain_notes[pointer + k]["start"], 
                            end=offset_time + bgn_sec, 
                            velocity=sustain_notes[pointer + k]["velocity"]
                        )
                        all_notes.append(note)  
                        

                    elif name == "name=note_sustain":

                        new_sustain_notes.append(sustain_notes[pointer + k])

                pointer += batch_size

            sustain_notes = new_sustain_notes
                ...
                for k in range(len(pred_tokens)):
                    ...
                    if name == "name=note_off":
                        ...
                        note = pretty_midi.Note(
                            ...
                            velocity=candidate_notes[pointer + k].velocity
                        )
                        all_notes.append(note)    
