...
def inference_in_batch(args):
    ...
    checkpoint_path = Path("checkpoints/train_llama_mt_on3/AudioLlama/step=50000_encoder.pth")
    ...
    # Load checkpoint
    checkpoint_path = Path("checkpoints/train_llama_mt_on3/AudioLlama/step=50000.pth")
    config = LLaMAConfig(
    ...
    # for audio_idx in range(110, len(audio_paths)):
        ...
        while bgn < audio_samples:
            ...
            with torch.no_grad():
                enc_model.eval()
                audio_emb = enc_model(segments)["reg_onset_output"]
