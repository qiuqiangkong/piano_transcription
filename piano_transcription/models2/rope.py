...
class RoPE(nn.Module):
    ...
    def apply_nd(self, x: Tensor, pos: LongTensor) -> Tensor:
        r"""Apply Nd RoPE with explicit positions.
...
if __name__ == '__main__':
    ...
    # Example 1
    x = torch.rand((B, L, N, H))  # (b, t, n, h)
    rope = RoPE(head_dim=H)
    out1 = rope(x)  # (b, t, n, h)

    # Example 2
    pos = torch.arange(L)[:, None]  # (l, 1)
    out2 = rope.apply_nd(x, pos)  # (b, t, n, h)

    # Example 3: RoPE Nd with explict positions
    K = 3  # Nd rope
    x = torch.rand((B, 5, N, H * K))
    pos = torch.LongTensor([[1, 1, 1], [2, 2, 2], [3, 3, 3], [4, 4, 4], [5, 5, 5]])
    out3 = rope.apply_nd(x, pos)  # (b, t, n, h)
