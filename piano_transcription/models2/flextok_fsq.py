...
class FlextokFSQ(nn.Module):
    def __init__(self, config): 
        ...
        self.enc_register = nn.Parameter(0.1 * torch.randn(self.register_size, config.n_embd))
        self.enc_transformer = nn.ModuleList(Block(config) for _ in range(config.n_layer))
        self.enc_proj2 = nn.Linear(config.n_embd, 6)

        levels = [4, 4, 4, 4, 4, 4]
        self.fsq = FSQ(levels)

        self.dec_proj2 = nn.Linear(6, config.n_embd)
        self.dec_register = nn.Parameter(0.1 * torch.randn(self.patch_frames, config.n_embd))
    ...
    def forward(
        ...
        # Pre layers
        x = rearrange(x, 'b c (t1 t2) f -> (b t1) t2 (c f)', t2=self.patch_frames)
