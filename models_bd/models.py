...
class Regress_onset_offset_frame_velocity_CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        reg_offset_output, offset_emb_h = self.reg_offset_model(x)    # (batch_size, time_steps, classes_num)
        velocity_output, _ = self.velocity_model(x)    # (batch_size, time_steps, classes_num)
        ...
        output_dict = {
            ...
            'onset_emb_h': onset_emb_h,
            'frame_emb_h': frame_emb_h,
            'offset_emb_h': offset_emb_h,
        }
...
class Note_pedal(nn.Module):
    ...
    def forward(self, input):
        ...
        full_output_dict["onoffvel_emb"] = torch.cat((full_output_dict["reg_onset_output"], full_output_dict["reg_offset_output"], full_output_dict["frame_output"]), dim=-1)

        full_output_dict["onoffvel_emb_h"] = torch.cat((full_output_dict["onset_emb_h"], full_output_dict["offset_emb_h"], full_output_dict["frame_emb_h"]), dim=-1)
